...
class M_penjualan_master extends CI_Model
{
	...
	{
		return $this->db
			->select('MAX(id_penjualan_m) as ids') 
			->limit(1)
			->get('pj_penjualan_master');
	}
	...
	function hapus_transaksi($id_penjualan, $reverse_stok)
	{
		if($reverse_stok == 'yes'){
			...
			{
				$sql = "
					UPDATE `pj_menu` SET `total_stok` = `total_stok` + ".$b->jumlah_beli." 
					WHERE `id_menu` = '".$b->id_menu."' 
				";
		...
		$sql = "
			SELECT 
				...
				(
					...
					LIMIT 1
				) AS total_penjualan, COUNT(id_penjualan_m) as tot
			FROM 
				`pj_penjualan_master` AS a 
			WHERE 

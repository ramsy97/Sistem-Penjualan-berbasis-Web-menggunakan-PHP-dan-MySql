...
class Penjualan extends MY_Controller 
{
	...
	public function transaksi()
	{
		...
		if($level == 'admin' OR $level == 'kasir')
		{
			if($_POST)
			{
				if( ! empty($_POST['kode_menu']))
				{
					...
					if($total > 0)
					{
						...
						$no = 0;
						foreach($_POST['kode_menu'] as $d)
						{
							if( ! empty($d))
							{
								$this->form_validation->set_rules('kode_menu['.$no.']','Kode Menu #'.($no + 1), 'trim|required|max_length[40]|callback_cek_kode_menu[kode_menu['.$no.']]');
						...
						$this->form_validation->set_message('cek_kode_menu', '%s tidak ditemukan');
								...
								if($master)
								{
									...
									foreach($_POST['kode_menu'] as $k)
									{
										if($k != null)
										{
											...
											$id_menu		= $this->m_menu->get_id($kode_menu)->row()->id_menu;
											
											$insert_detail	= $this->m_penjualan_detail->insert_detail($id_master, $id_menu, $jumlah_beli, $harga_satuan, $sub_total);
 											if($insert_detail)
											{
												$this->m_menu->update_stok($id_menu, $jumlah_beli);
												$inserted++;
									...
									{
										$this->query_error(json_encode($this->db->error()));
									}
								}
								else
								{
			...
			else
			{
				...
				$this->load->model('m_penjualan_master');
				$dt['transaksi'] = $this->m_penjualan_master->get_last_id();
	...
	public function transaksi_cetak()
	{
		...
		foreach($_GET['kode_menu'] as $kd)
		{
			if( ! empty($kd))
			{
				$nama_menu = $this->m_menu->get_id($kd)->row()->nama_menu;
				$nama_menu = character_limiter($nama_menu, 20, '..');

				$pdf->Cell(25, 5, $kd, 0, 0, 'L');
				$pdf->Cell(40, 5, $nama_menu, 0, 0, 'L');
	...
	public function ajax_kode()
	{
		if($this->input->is_ajax_request())
		{
			...
			$menu = $this->m_menu->cari_kode($keyword, $registered);
			...
			if($menu->num_rows() > 0)
			{
				...
				foreach($menu->result() as $b)
				{
					$json['datanya'] .= "
						<li>
							<b>Kode</b> : 
							<span id='kodenya'>".$b->kode_menu."</span> <br />
							<span id='menunya'>".$b->nama_menu."</span>
